...
Let us further assume that the measurements took place at baseline (time $t = 0$),
after $t = 1$ week, $t = 5$ weeks, $t = 7$ weeks, and $t = 11$ weeks. With 
linear latent growth curve models,
the observations at the four measurement occasions are predicted with a latent intercept (`I`) and a latent slope (`S`) using the equation 
$$y_t = I + S*t+\varepsilon$$ 
...
the slope `S`. More precisely, we assume that `I` and `S` are random variables and 
`I` may be `.4` for the first person, but `-1.2` for the second. SEMs allow us
...
(`I`) to the observations (`y1`-`y5`) are constrained to `1`, while the paths of
the latent slope (`S`) are set to the time points
$0$, $1$, $5$, $7$, and $11$. Because `I` and `S` are modeled as latent variables
